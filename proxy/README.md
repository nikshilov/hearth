...
Tiny HTTP bridge from the Heart browser to the local `claude` CLI, so Heart chat turns go through your **Max subscription** instead of the Anthropic API.

## Why

Heart's browser-side `ClaudeAdapter` calls Anthropic with an API key. That bills the API account directly. If you already pay for Claude Max, you'd rather not double-pay.

This proxy:

- listens on `127.0.0.1:18791`
- accepts `POST /chat` from Heart (`http://localhost:5173`)
...
HEARTH_PROXY_CWD=/tmp/heart-claude-cwd
...
- **No streaming yet** — returns the full message at once. Heart `LocalClaudeAdapter` emits one `onChunk` to keep the UI shape compatible.
